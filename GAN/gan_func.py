...
def train_discriminator_step(model_components, seed=0):
    ...
    [cache_prefix, with_class, starting_step, train, data_cols, data_dim, label_cols, label_dim,
    ...
    if with_class:
        d_l_g, d_l_r, _ = sess.run([_disc_loss_generated, _disc_loss_real, disc_optimizer], feed_dict={
            ...
            _x: np.hstack((get_data_batch(train, batch_size, seed=seed), get_data_batch(train, batch_size, seed=seed)[:, -label_dim:])),
            _labels: get_data_batch(train, batch_size, seed=seed)[:,-label_dim:], # .reshape(-1, label_dim),
...
def adversarial_training_WGAN(arguments, train, data_cols, label_cols=[], seed=0, starting_step=0):
    ...
    model_components = [cache_prefix, with_class, starting_step,
                        train, data_cols, data_dim, label_cols, label_dim,

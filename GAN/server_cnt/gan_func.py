...
def training_steps_GAN(model_components):
    ...
    for i in range(starting_step, starting_step + nb_steps):
        ...
        for j in range(k_d):
            ...
            if with_class:
                x = np.hstack((get_data_batch(train, batch_size, seed=i+j), get_data_batch(train, batch_size, seed=i+j)[:, :1]))
                labels = x[:, :1]
                g_z = generator_model.predict([z, labels])
            else:
                g_z = generator_model.predict(z)
        ...
        if not i % 10:                      # 2x faster than testing each step...
            K.set_learning_phase(0)         # 0 = test
            test_size = 1500                # test using all of the actual fraud data
            x = get_data_batch(train, test_size, seed=i)
            z = np.random.normal(size=(test_size, rand_dim))
            if with_class:
                x = np.hstack((get_data_batch(train, test_size, seed=i), get_data_batch(train, test_size, seed=i)[:, :1]))
                labels = x[:,:1]
                g_z = generator_model.predict([z, labels])
            else:
                g_z = generator_model.predict(z)

...
with open(new_name + '.txt', 'w') as text_file:
    ...
    for index, row in data.iterrows():
        if agent == 0 and row['src'] == server_ip:
            ...
            # text_file.write('%s %s %s %s\n' % (str(index).ljust(5), agent_src[agent], str(server_cnt).ljust(2), str(row['times'] - server_time)))
            # text_file.write('%s %s\n' % (str(server_cnt), str(row['times'] - server_time)))
            ...
            # text_file.write('%s %s %s %s\n' % (str(index).ljust(5), agent_src[agent], str(client_cnt).ljust(2), str(row['times'] - client_time)))
            if row['times'] - client_time < 30000:
                text_file.write('%s %s\n' % (str(client_cnt), str(row['times'] - client_time)))

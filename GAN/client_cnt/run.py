...
def test_model(agent, train, dataSample):
    ...
    path = '../Desktop/' + agent + '_cnt/WCGAN_generator_model_weights_step_25000.h5'
...
def main(agent, train, dataSample, train_2, dataSample_2, loop_size):
    ...
    path_cnt = '../Desktop/' + agent + '_cnt/WCGAN_generator_model_weights_step_25000.h5'
    ...
    while cnt < loop_size:   
        packet_info = ''
        cnt += 1
        return_times, seed = generate_times(train_2, dataSample_2, seed, noise_pool, generator_model_cnt)
        if return_times not in times_cnt:
            times_cnt[return_times] = 1
        else:
            times_cnt[return_times] += 1
        times_total += 1
        ...
        with open(agent + '_rec.txt', 'a+') as a:
